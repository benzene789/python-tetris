...
class Board:
    ...
    def draw_shape_info(self, next_shape, display, x, y, text):
        # check for held shape
        if next_shape is not None:
            # draw the label
            FONT = pygame.font.SysFont("Arial", 20)
            next_shape_text = FONT.render(f"{text}" , True, pygame.Color("red"))
